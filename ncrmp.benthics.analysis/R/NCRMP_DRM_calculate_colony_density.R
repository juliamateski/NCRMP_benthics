...
NCRMP_DRM_calculate_colony_density <- function(project = "NULL", region, species_filter = "NULL") {
  ...
  #unpack list (old way was for(k in 1:length...))
 ...
 ####Helper function to clean the data####
  ...
  if (project == "NCRMP_DRM" || (project == "NCRMP" && (region == "SEFCRI" || region == "Tortugas"))) {
      ...
      density_species <- dat_1stage %>%
        clean_data() %>% # Change PROT to factor for ggplot will recognize it as a grouping variable
      ...
      density_species <- pivot_function(
        data = density_species,
        vars = c("REGION", "SURVEY", "YEAR", "SUB_REGION_NAME", "ADMIN", "PRIMARY_SAMPLE_UNIT", "LAT_DEGREES", "LON_DEGREES", "STRAT", "HABITAT_CD", "PROT", "MIN_DEPTH", "MAX_DEPTH", "PROT"),
        spp = spp) %>%
